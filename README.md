...
A Prometheus exporter for detailed btrfs filesystem metrics. Collects subvolume sizes, qgroup accounting, commit stats, replace/balance/scrub progress, bees dedup stats, orphan tracking, and more.

5x faster than equivalent bash/python scripts, with 100% metric parity.
...
- **`/run/btrfs-thread-map`** — for `btrfs_commit_running` detection, generated by [btrfs-thread-map.sh](https://gist.github.com/Finomosec/btrfs-thread-map)
...
| Commit running (D-state) | `/proc/<pid>/stat` + `/run/btrfs-thread-map` | procfs read |
...
- **`btrfs_commit_running`** relies on `/run/btrfs-thread-map` for PID-to-filesystem mapping. Without this file, commit running detection is disabled. The map can be generated by [btrfs-thread-map.sh](https://gist.github.com/Finomosec/btrfs-thread-map) or by calling `btrfs-thread-map.sh` from a timer/cron.
